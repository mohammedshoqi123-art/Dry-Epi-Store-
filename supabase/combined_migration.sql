...
-- ╔═══════════════════════════════════════════════════════════╗
-- ║  PART 9: Yemen Governorates (13 محافظة)                 ║
-- ╚═══════════════════════════════════════════════════════════╝
...
-- ╔═══════════════════════════════════════════════════════════╗
-- ║  PART 10: Yemen Districts (122 مديرية)                  ║
-- ╚═══════════════════════════════════════════════════════════╝
...
INSERT INTO districts (governorate_id, name_ar, name_en, code) SELECT id, 'ميفعه', 'Mayfa''a', 'SHABWA-MAYFAA' FROM governorates WHERE code = 'SHABWA' ON CONFLICT (code) DO NOTHING;
...
-- ╔═══════════════════════════════════════════════════════════╗
-- ║  PART 11: SIA Forms (نماذج النشاط الإيصالي)            ║
-- ╚═══════════════════════════════════════════════════════════╝
...
-- ╔═══════════════════════════════════════════════════════════╗
-- ║  PART 12: Yemen Health Facilities (1483 مرافق صحي)      ║
-- ╚═══════════════════════════════════════════════════════════╝
...
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'المركز الصحي الحوطة', 'Al Hawtah HC', 'SHABWA-MAYFAA'-ALHAWTAHHC' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية الجريبة', 'Alguribah HU', 'SHABWA-MAYFAA'-ALGURIBAHH' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية الحضن', 'Alhathan HU', 'SHABWA-MAYFAA'-ALHATHANHU' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'مركز الامومه والطفولة عزان', 'Azan Maternity & Childhood Center', 'SHABWA-MAYFAA'-AZANMATERN' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية باعرام', 'Baram HU', 'SHABWA-MAYFAA'-BARAMHU' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'المركز الصحي جول الريدة', 'Jawl Al Raydah HC', 'SHABWA-MAYFAA'-JAWLALRAYD' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية جول الشيخ', 'Jawl Al Sheikh HU', 'SHABWA-MAYFAA'-JAWLALSHEI' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية ميفعة القديمة', 'Mayfa''a Al Qadeema HU', 'SHABWA-MAYFAA'-MAYFA''''AAL' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية قبائل السمر', 'Qabayl Alsomr HU', 'SHABWA-MAYFAA'-QABAYLALSO' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية قلقل', 'Qalqal HU', 'SHABWA-MAYFAA'-QALQALHU' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية صعيد باقادر', 'Saeed Ba Qader HU', 'SHABWA-MAYFAA'-SAEEDBAQAD' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
INSERT INTO health_facilities (district_id, name_ar, name_en, code) SELECT d.id, 'الوحدة الصحية طبق', 'Tabaq HU', 'SHABWA-MAYFAA'-TABAQHU' FROM districts d JOIN governorates g ON d.governorate_id = g.id WHERE d.code = 'SHABWA-MAYFAA ON CONFLICT (code) DO NOTHING;
...
-- ============================================================
-- ✅ DONE! 13 محافظة يمنية + 122 مديرية + 1483 مرافق صحي + 5 نماذج
-- ============================================================
